    ...
    <div className="relative bg-[var(--neutral-0)] rounded-[var(--radius-xl)] shadow-sm border border-[var(--border-300)] flex flex-col justify-between h-full hover:shadow-md transition-shadow overflow-hidden group">
      ...
      <div className={isVip ? "filter blur-[6px] opacity-35 select-none pointer-events-none flex flex-col justify-between h-full" : "flex flex-col justify-between h-full"}>
        ...
        <div className="p-5 flex gap-3.5 border-b border-[var(--border-200)] relative bg-[var(--surface-200)] min-h-[128px] items-start">
          {/* Icon block */}
          <div className={`w-11 h-11 rounded-[var(--radius-md)] flex-shrink-0 flex items-center justify-center !text-white font-black text-lg ${iconBg} relative z-10 shadow-xs`}>
            {icon}
          </div>
          
          <div className="relative z-10 flex flex-col justify-between h-full">
            <h3 className="text-body-lg font-bold !text-[var(--text-primary-500)] leading-snug mb-2 line-clamp-2 min-h-[42px]">
              {title}
            </h3>
            <div className="flex flex-wrap gap-1.5 mt-auto">
              <span className="px-2 py-0.5 bg-[var(--neutral-0)] border border-[var(--border-300)] !text-[var(--text-secondary-600)] text-caption font-extrabold uppercase rounded-md tracking-wider">
                {type}
              </span>
              <span className="px-2 py-0.5 bg-[var(--neutral-0)] border border-[var(--border-300)] !text-[var(--text-secondary-600)] text-caption font-extrabold uppercase rounded-md tracking-wider">
                {tag}

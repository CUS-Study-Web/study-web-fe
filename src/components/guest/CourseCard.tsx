  ...
  return (
    <div className="bg-white rounded-[var(--radius-xl)] overflow-hidden shadow-lg border border-[var(--border-300)] flex flex-col group hover:shadow-xl transition-all duration-300">
      ...
      <div className="relative h-56 w-full">
        ...
        <div className={`absolute top-4 left-4 ${btnColor} !text-white text-xs font-extrabold px-3.5 py-1.5 rounded-full shadow-xs tracking-wide uppercase`}>
          {tag}
        </div>

        {/* Title & Subtitle - Bright Pure White & Light Tint Text */}
        <div className="absolute bottom-4 left-4 right-4 text-white">
          <h3 className="text-h3 font-black tracking-wide mb-1 !text-white drop-shadow-sm">
            {title}
          </h3>
          <p className="text-body-sm font-semibold !text-[#e2e8e3] line-clamp-1 drop-shadow-sm">
            {subtitle}
          </p>
        </div>
      </div>
      ...
      <div className="p-6 flex flex-col flex-grow justify-between gap-6">
        <p className="text-body-sm !text-[var(--text-secondary-600)] leading-relaxed line-clamp-4 min-h-[80px] font-medium">
          {desc}
        </p>
        <Link

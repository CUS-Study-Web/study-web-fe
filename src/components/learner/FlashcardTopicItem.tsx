...
export default function FlashcardTopicItem({ topic }: FlashcardTopicItemProps) {
  ...
  const pct = Math.round((topic.learned / topic.total) * 100);
  const done = topic.learned === topic.total;

  return (
    <div 
      className="bg-white rounded-[18px] border border-[var(--border-300)] shadow-sm p-5 flex flex-col justify-between min-h-[200px] transition-all duration-150 hover:shadow-md hover:-translate-y-1"
    >
      <div className="flex-1">
        ...
        <div className="font-[family:var(--font-body)] text-[11px] text-[var(--text-secondary-200)]">{topic.total} từ vựng</div>
      </div>
      <div className="mt-4">
        ...
        <div className="flex justify-between mb-3.5">
          <span className="font-[family:var(--font-body)] text-[11px] text-[var(--text-secondary-400)]">Đã nhớ {topic.learned}/{topic.total}</span>
          <span className="font-[family:var(--font-heading)] font-bold text-[11px] text-[var(--brand-base-500)]">{pct}%</span>
        </div>
        <button 
          onClick={() => navigate(ROUTES.LEARNER.FLASHCARD_STUDY(String(topic.id)))}
